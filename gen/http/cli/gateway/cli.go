...
Usage:
    %s [globalflags] gcp COMMAND [flags]
...
func gcpPublishUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] gcp publish -body JSON -topic STRING
...
Usage:
    %s [globalflags] healthz COMMAND [flags]
...
Additional help:
    %s healthz COMMAND --help
`, os.Args[0], os.Args[0])
}
func healthzReadinessUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] healthz readiness
...
func healthzLivenessUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] healthz liveness

サーバが生きているかチェックするエンドポイント

Example:
    `+os.Args[0]+` healthz liveness
`, os.Args[0])
}
